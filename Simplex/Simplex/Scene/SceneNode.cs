...
namespace Simplex.Scene
{
    ...
    public class SceneNode : IDisposable
    {
        ...
        public Vector3 Forward { get { return Vector3.Normalize( Vector3.Transform(Vector3.UnitZ * -1,_rotation)); } }

        public Vector3 Right { get { return Vector3.Normalize( Vector3.Transform(Vector3.UnitX,_rotation)); } }

        public Vector3 Up { get { return Vector3.Normalize( Vector3.Transform(Vector3.UnitY,_rotation)); } }
        ...
        public void RotateX(float angle)
        {
            ...
            Rotation = Quaternion.Normalize(_rotation * rot);
        }
        ...
        public void RotateY(float angle)
        {
            foreach (SceneNode child in _childNodes)
                child.RotateY(angle);
            Quaternion rot = Quaternion.CreateFromAxisAngle(Up, angle);
            Rotation = Quaternion.Normalize(_rotation * rot);
        }
        ...
        public void RotateZ(float angle)
        {
            foreach (SceneNode child in _childNodes)
                child.RotateZ(angle);
            Quaternion rot = Quaternion.CreateFromAxisAngle(Forward, angle);
            Rotation = Quaternion.Normalize(_rotation * rot);
        }
        ...
        public virtual void LookAt(in Vector3 target)
        {
            ...
            if((target-_translation).Length()<0.1f)
              return;
            Vector3 direction =Vector3.Normalize(target - _translation);
            Vector3 forward = Vector3.UnitZ * -1;
           ...
           //    rotAxis*=-1;
            ...
            Vector3 u = Vector3.Normalize(rotAxis);
