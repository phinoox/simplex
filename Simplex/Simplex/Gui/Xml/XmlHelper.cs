			...
			{ typeof(Font), (element, value) =>
				{
					...
					if (element is Simplex.Gui.Control.ControlBase)
						return Font.Create(((Simplex.Gui.Control.ControlBase)element).Skin.Renderer, name, size, style);
					else if (element is Simplex.Gui.Xml.Component)
						return Font.Create(((Simplex.Gui.Xml.Component)element).View.Skin.Renderer, name, size, style);
					else
